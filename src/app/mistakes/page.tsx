...
export default function MistakesPage() {
  ...
  useEffect(() => setIds(getUnresolvedMistakeIds(readProgress())), []);
  const questions = demoQuestions.filter((question) => ids.includes(question.id));

  return (
    <div className="page">
      <header className="hero compact"><p className="eyebrow">ПОВТОРЕНИЕ</p><h1>Ошибки</h1><p>В MVP вопрос остаётся в очереди, пока после последней ошибки не будет получено два правильных ответа.</p></header>
      {questions.length === 0 ? <div className="empty-state"><h2>Пока нечего повторять</h2><p>Ошибочные вопросы появятся здесь автоматически.</p><Link className="button" href="/practice">Начать тренировку</Link></div> : <>
